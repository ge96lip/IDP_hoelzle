...
model_gamlss <- "GAMLSS"
merged <- read.csv(file="~/Desktop/SS2024/IDP/data/data_merged.csv")
#lh_columns <- names(data)[grepl("^(lh|rh)_.*thickness$", names(data))]
data_patients <- read.csv(file="~/Desktop/SS2024/IDP/data/data_patients.csv")
data_train <- read.csv(file="~/Desktop/SS2024/IDP/data/train_healthy.csv")
data_test <- read.csv(file="~/Desktop/SS2024/IDP/data/test_healthy.csv")
...
data_patients <- data[data$DX %in% c("AD"),]
data_healthy <- data[data$DX %in% c("CN"),]
...
generate_plots_gam <- function(models, data_healthy, resultsDir, suffix, data_patients, sd_pop, gender_choice, xmin = 50, xmax = 90, outlier_thresh = 7) {
  ...
  # Ensure xmin and xmax are single values
  xmin <- as.numeric(xmin)
  xmax <- as.numeric(xmax)
  
  # Create dummy data for the chosen gender with necessary columns
  dummy_ages <- seq(xmin, xmax, 0.5)
  dummy_gender <- factor(rep(gender_choice, length(dummy_ages)))
  data_dummy <- data.frame(AGE = dummy_ages, PTGENDER = dummy_gender)
  
  # Add other necessary columns with default or mean values from data_healthy
  necessary_columns <- setdiff(names(data_healthy), names(data_dummy))
  for (col in necessary_columns) {
    if (is.numeric(data_healthy[[col]])) {
      data_dummy[[col]] <- mean(data_healthy[[col]], na.rm = TRUE)
    } else if (is.factor(data_healthy[[col]])) {
      data_dummy[[col]] <- factor(unique(data_healthy[[col]])[1])
    } else {
      data_dummy[[col]] <- unique(data_healthy[[col]])[1]
    }
  }
  ...
  for (roi in names(models)) {
    model <- models[[roi]]
    pred <- predict(model, newdata = data_dummy, se.fit = TRUE)
    ...
    plot_data <- data.frame(
      ...
      SD = predicted_sds,
      ROI = roi,
      PTGENDER = dummy_gender
    )
    
    plot_data_list[[roi]] <- plot_data
    ...
    true_data <- data.frame(
      ...
      ROI = roi,
      PTGENDER = filtered_data$PTGENDER
    )
    patient_data_list[[roi]] <- true_data
  }
  ...
  combined_patient_data <- do.call(rbind, patient_data_list)
  
  # Set colors based on gender
  color_scale <- scale_color_manual(values = c("Male" = "blue", "Female" = "red"))
  fill_scale <- scale_fill_manual(values = c("Male" = "blue", "Female" = "red"))
  
  dynamic_title <- paste("Predictions GAM with Confidence Intervals -", gender_choice)
  ggplot(combined_plot_data, aes(x = AGE)) +
    geom_line(aes(y = Prediction), color = ifelse(gender_choice == "Male", "blue", "red")) +
    geom_ribbon(aes(ymin = Lower_05, ymax = Upper_05), fill = ifelse(gender_choice == "Male", "blue", "red"), alpha = 0.1) +
    geom_ribbon(aes(ymin = Lower_25, ymax = Upper_25), fill = ifelse(gender_choice == "Male", "blue", "red"), alpha = 0.1) +
    geom_point(data = combined_patient_data, aes(x = AGE, y = TrueValue), color = ifelse(gender_choice == "Male", "blue", "red"), size = 1.5, shape = 21) +
...
# Function for plotting gamlss
generate_plots_gamlss <- function(models, data_healthy, resultsDir, suffix, data_patients, gender_choice, xmin = 50, xmax = 90, outlier_thresh = 7) {
  remove_outliers <- function(values, threshold = 7) {
    z_scores <- (values - mean(values, na.rm = TRUE)) / sd(values, na.rm = TRUE)
    return(abs(z_scores) < threshold & values > 0)
  }
  ...
  for (roi in names(models)) {
    ...
    pred <- predict(model, newdata = data_dummy, what = "mu", type = "response")
    predicted_sigma <- predict(model, newdata = data_dummy, what = "sigma", type = "response")
  ...
  color_scale <- scale_color_manual(values = c("Male" = "blue", "Female" = "red"))
  fill_scale <- scale_fill_manual(values = c("Male" = "blue", "Female" = "red"))
  dynamic_title <- paste("Predictions GAMLSS with Confidence Intervals -", gender_choice)
  ggplot(combined_plot_data, aes(x = AGE)) +
    geom_line(aes(y = Prediction), color = ifelse(gender_choice == "Male", "blue", "red")) +
    geom_ribbon(aes(ymin = Lower_05, ymax = Upper_05), fill = ifelse(gender_choice == "Male", "blue", "red"), alpha = 0.1) +
    geom_ribbon(aes(ymin = Lower_25, ymax = Upper_25), fill = ifelse(gender_choice == "Male", "blue", "red"), alpha = 0.1) +
...
outlier_thresh <- 7
generate_plots_gam(gam_models, data_healthy = data_train, resultsDir=resultsDir, suffix="GAM", data_patients = data_patients, sd_pop=sd_pop, gender_choice = "Male")

generate_plots_gam(gam_models, data_healthy = data_train, resultsDir=resultsDir, suffix="GAM", data_patients = data_patients, sd_pop=sd_pop, gender_choice = "Female")
...
generate_plots_gamlss(gamlss_models, data_healthy = data_train, resultsDir=resultsDir, suffix="GAMLSS", data_patients = data_patients, gender_choice = "Female")
generate_plots_gamlss(gamlss_models, data_healthy = data_train, resultsDir=resultsDir, suffix="GAMLSS", data_patients = data_patients, gender_choice = "Male")
