...
resultsDir <- 'path/to/result/directory'
...
generate_plots_gam <- function(models, data_healthy, resultsDir, suffix, data_patients, sd_pop, gender_choice, xmin = 50, xmax = 90, outlier_thresh = 7) {
  ...
  print(dim(patient_data_list))
  combined_patient_data <- do.call(rbind, patient_data_list)
  print(dim(combined_patient_data))
  ...
  ggplot(combined_plot_data, aes(x = AGE)) +
    ...
    geom_point(data = combined_patient_data, aes(x = AGE, y = TrueValue, color = ROI), size = 1.5, shape = 21) +
    labs(title = "Predictions with Data Variance and Confidence Intervals",
         x = "Age",
         y = "Thickness") +
